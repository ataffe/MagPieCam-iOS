...
//        config.iceServers = [RTCIceServer(urlStrings: ["stun:stun.l.google.com:19302"])]
        ...
        pc.offer(for: offerConstraints) { [weak self] sdp, error in
            guard let self, let sdp else {
                Logger.whepClient.error("WHEP: offer failed: \(error?.localizedDescription ?? "unknown error")")
                return
            }
            pc.setLocalDescription(sdp) { error in
                if let error {
                    Logger.whepClient.error("WHEP: setLocalDescription failed: \(error.localizedDescription)")
                    return
                }
